...
const getStatus = (writeStatus: Status, confirmationStatus: Status): Status => {
  ...
  if (writeStatus === 'loading' || confirmationStatus === 'loading') return 'loading'

  if (writeStatus === 'error' || confirmationStatus === 'error') return 'error'

  if (writeStatus === 'success' && confirmationStatus === 'success') return 'success'

  return 'idle'
}

type PrepareFields = 'args' | 'address' | 'abi' | 'functionName' | 'chainId' | 'overrides'
...
type Config<TAbi extends Abi, TFunctionName extends string> = PrepareArgs<TAbi, TFunctionName> &
  WriteArgs<TAbi, TFunctionName> &
  WaitArgs

export const useContractWrite = <TAbi extends Abi, TFunctionName extends string>(

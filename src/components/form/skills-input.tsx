...
import { MINIMUM_QUERY_LENGTH, useWorkxSkills } from '../modules/workx/hooks/use-workx-skills'
  ...
  const renderSkills = allSkills.map((skill: string) => {
    return (
      <div
        key={skill}
        className={
          'mr-2 mb-1 inline-flex items-center rounded-full bg-indigo-100 pl-3 py-0.5 text-sm font-medium text-blue-800'
        }
      >
        <span>{skill}</span>
        <span

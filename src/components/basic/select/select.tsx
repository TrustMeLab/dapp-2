...
const iconClass = 'absolute top-1/2 transform -translate-y-1/2 [&>svg]:max-w-min'
...
export const Select = ({
  ...
  return (
    <div className={cx('flex-col relative', block ? 'flex' : 'inline-flex', className)}>
              ...
              {items.map((item) => (
                ...
                >
                  {({ selected }) => (
                    <>
                      <span
                        className={cx('block truncate', selected ? 'font-medium' : 'font-normal')}
                      >
                        {item}
                      </span>
                      {/* Check Icon */}
